...
namespace AIGraph.Editor
{
    public class AIGraphSaveUtility
    {
        ...
        {
            var graphData = ScriptableObject.CreateInstance<AIGraphDataContainer>();
        ...
        public static bool SaveGraph(AIGraphView graphView)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                if (node is RootNode r)
                    graphView.dataContainer.AIID = r.AIID;
        ...
        /// </summary>
        public static void SaveWindowInfo(AIGraphView graphView)
        {
            // Viewのポジションとズームを保存
        ...
        /// </summary>
        /// <param name="path"></param>
        public static AIGraphView LoadGraph(string path)
        {
            if (!File.Exists(path)) return null;
            var graphView = new AIGraphView();
        ...
        /// <param name="path"></param>
        /// <param name="graphView"></param>
        public static void LoadGraph(string path, AIGraphView graphView)
        {
            var dataContainer = AssetDatabase.LoadAssetAtPath<AIGraphDataContainer>(path);
        ...
        /// <param name="graphView"></param>
        public static AIGraphView LoadGraph(AIGraphDataContainer dataContainer, AIGraphView graphView = null)
        {
            if (graphView == null)
            ...
            AIGraphView output = graphView;
            return output;
        }

        private static void ClearGraph(AIGraphView graphView)
        {
            graphView.nodes.ToList().ForEach(graphView.RemoveElement);
            graphView.edges.ToList().ForEach(graphView.RemoveElement);
        }

        private static void CreateNodes(AIGraphView graphView, AIGraphDataContainer graphData)
        {
        ...
        private static void CreateEdges(AIGraphView graphView, AIGraphDataContainer graphData)
        {
            var nodes = GetNodes(graphView);
