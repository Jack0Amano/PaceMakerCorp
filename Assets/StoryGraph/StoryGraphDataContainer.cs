...
namespace StoryGraph.Editor
{
    ...
    /// </summary>
    [Serializable]
    public class StoryGraphDataContainer : ScriptableObject
    {
        ...
        public List<EventGraph.Editor.EventGraphDataContainer> AllEventsData
        {
            get
            {
                var output = Nodes.ConvertAll(n =>
                {
                    if (n.raw.GetFromPairs(EventNode.EventNodeFileKey, out UnityEngine.Object file))
                        return (EventGraph.Editor.EventGraphDataContainer)file;
                    return null;
                });
                output.RemoveAll(d => d == null);
                return output;
            }
        }
        ...
        public static bool OnOpenAsset(int instanceID, int line)
        {
            ...
            if (target is StoryGraphDataContainer)
            {
                ...
                bool windowIsOpen = EditorWindow.HasOpenInstances<StoryGraphWindow>();
                StoryGraphWindow window;
                if (!windowIsOpen)
                {
                    window = EditorWindow.CreateWindow<StoryGraphWindow>();
                }
                else
                {
                    EditorWindow.FocusWindowIfItsOpen<StoryGraphWindow>();
                    window = (StoryGraphWindow)EditorWindow.focusedWindow;
                }
